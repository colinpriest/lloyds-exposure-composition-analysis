...
def md(c, r):
    ...
    A(f"- **Working-sample exclusions:** of the {c['excl']} corpus records dropped, "
      ...
      f"filing shows to be dominated by a transfer (the reserves or premium a transfer brought in, "
      f"or the commutation of one back to its cedant), which is not development, {c['wt']} lack "
      f"usable LoB weights ({c['unrec']} of them because the classes of the adopted mix, with their "
    ...
    A("- **RITC handling.** External RITC injects a lumpy, non-recurring step into prior-year "
      ...
      f"$P(|\\beta_{{\\text{{RITC}}}}|>0.1)={p_abs:.2f}$); the transfer operator omits that "
      "multiplier as a structural simplification, not because it was shown to be zero, at an "
      f"assessed vignette cost of {cost}. The "
