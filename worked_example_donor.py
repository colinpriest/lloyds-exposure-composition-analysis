...
Selects two REAL donors from the 491-donor pool per the spec (Donor A: large adverse
movement from a small syndicate; Donor B: moderate adverse movement from a target-size but
more-concentrated syndicate), and runs the five-step transfer arithmetic with a point
estimate (posterior mean) and a 95% interval (propagating posterior draws of theta through
the rescaling factor). Computation at full precision; display rounded to 3 sig figs.

Uses the same scale function and parameters as the aggregate pipeline (dispersion_adjustment
in run_analysis.py and vignette_uncertainty.py), so the single-donor arithmetic is consistent
with the aggregate Vignette 1 numbers.
...
def load_params():
    cal = json.loads((SCRIPT_DIR / "dispersion_calibration.json").read_text())
    z = np.load(SCRIPT_DIR / "dispersion_posterior_draws.npz")
    mean = {p: float(cal[p]) for p in ("k", "gamma", "sd_undiv", "sd_div")}
    mean["ref"] = float(cal["reference_size"]); mean["hlo"] = float(cal["hhi_floor"]); mean["hce"] = float(cal["hhi_ceil"])
    draws = {p: z[p] for p in ("k", "gamma", "sd_undiv", "sd_div")}
    return mean, draws
...
def transfer_detail(d, mp, draws):
    ...
    lam_d = sq / si; sq_d = lam_d * Si
    return {
        "syndicate": d["syndicate"], "year": d["year"],
