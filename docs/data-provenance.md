...
- **Confirmed inward transfers join the same regime** (PLAN R195): the hand-adjudicated
  ...
  interchangeable: **50** are in the **928-record corpus**, and **34** are in the
...
```
1065 files -> 928 corpus -> 691 modelling sample
  ...
  from the corpus to the working sample, also disjoint:
    928 - 125 net or unstated basis (101 net, 24 unstated; `data/pyd_basis_register.json`,
     `src/pyd_basis_rule.py`) - 4 take-on, not development
     (`data/takeon_not_development.json`) - 12 unusable severity - 0 missing opening reserves
     - 96 without premium weights = 691
...
<!-- coverage:start -->
Coverage is **66 % of active syndicate-years overall** (691 of 1040; it was ~47 % on the
old dataset), and it is **uneven, not flat**: the annual rate runs from **18 % in 2014 (17 of
92)** to **78 % in 2019**, with every other year between 61 % and 78 %. The 2020–2024 retrieval
...
sensitivity shows exactly this direction of effect (the floor moves from about 3.1% to
2.2% when partially pooled syndicate intercepts are added). This is distinct from the
