...
Severity ratios are **not winsorised** — the analysis preserves raw tail
behaviour to avoid conflicting with VaR 99.5% estimation. Winsorisation at the
95th percentile is applied only to squared-severity values used in the
dispersion model fitting (N5/N6), not to the main severity distributions.
...
### 3.4  Direct projection approach

Every source syndicate-year contributes to the adjusted distribution through
**direct projection**: its LoB severity vector $\mathbf{s}_\ell$ is dotted with
the query weight vector $\mathbf{w}^{(q)}$ regardless of the source's own LoB
mix. Unobserved LoB in the source contribute zero severity (§2.3).

This avoids the complexity of hierarchical matching — each historical
observation is a self-contained scenario once LoB severities are computed.

#### Donor similarity analysis

To validate that the adjustment is not driven by mix-dissimilar donors, a
**local-donor sensitivity analysis** progressively restricts the donor pool
using Hellinger distance between LoB weight vectors:

$$
H(\mathbf{w}^{(s)}, \mathbf{w}^{(q)}) = \frac{1}{\sqrt{2}} \left\|\sqrt{\mathbf{w}^{(s)}} - \sqrt{\mathbf{w}^{(q)}}\right\|_2
$$

At each distance threshold $H_{\max} \in \{0.30, 0.35, \ldots, 1.0\}$, only
donors with $H \le H_{\max}$ are retained, and VaR 99.5% is recomputed. As the
pool is restricted to mix-similar syndicates, the raw VaR should converge
toward the adjusted VaR — confirming that the adjustment correctly removes the
LoB-mix distortion.
...
When the **combined model** from N6 analysis is available, the `size_factor()`
function uses a 2D dispersion adjustment that accounts for both portfolio size
and HHI concentration simultaneously, replacing the simple power-law (see §11,
N6).
...
### 5.1  Stage 1: Overall coefficient via RE-GLS

The overall size coefficient $\bar{\beta}$ is estimated by **RE-GLS** (Random
Effects Generalised Least Squares) with syndicate random intercepts and event
fixed effects:

$$
s_{it} = \alpha + \bar{\beta} \cdot \ln(R_{it}) + \delta \cdot t + u_i + \sum_e \gamma_e \cdot \mathbf{1}[\text{event}_{it} = e] + \varepsilon_{it}
$$

where:

- $s_{it}$ is the severity ratio for syndicate $i$ in year $t$
- $R_{it}$ is the portfolio size (reserves) for syndicate $i$ in year $t$
- $u_i$ is the syndicate random intercept (captures persistent syndicate
  heterogeneity)
- $\text{event}_{it} = (\text{year}_{it}, \text{cause\_category}_{it})$
  identifies the common market event
- $\gamma_e$ are event fixed effects
- $\mathbf{1}[\cdot]$ is the indicator function

The syndicate random effects account for persistent differences between
syndicates (e.g. reserving philosophy), while the event fixed effects control
for confounding from common market shocks. Together they isolate the pure size
effect more cleanly than pooled OLS.

**Identification requirement:** An event must involve at least `min_events`
syndicates (default 3) to be included.

**Comparison models:** Four alternative specifications are also estimated for
robustness:

| Model | Specification | Purpose |
|-------|---------------|---------|
| **M0** | Simple OLS: $s_i = \alpha + \beta \ln R_i$ | Baseline (no controls) |
| **M1** | OLS + event FE | Controls for common events |
| **M2** | Event FE on $\log|S|$ | Log-scale severity |
| **M3** | Event FE on $|S|$ | Variance-scale severity (absolute severity) |
| **M1\_balanced** | M1 on BALANCED\_K8 subset | Balanced panel robustness |

The primary RE-GLS estimate is used for all downstream adjustments; the
comparison models appear in diagnostic tables.
...
All size coefficients are **data-driven** — there are no hardcoded default
$\beta$ values. The system initialises `LOB_BETA_COEFFICIENTS = {}` and
`OVERALL_BETA_DEFAULT = 0.0`, then populates them from the N3 analysis.

### 6.1  Overall coefficient

$$
\bar{\beta} = \text{observation-weighted mean of LoB betas from N3}
$$

If the N3 analysis has not yet run (or fails), the fallback is $\bar{\beta} = 0$
(i.e. no size adjustment).

### 6.2  LoB-specific coefficients

LoB coefficients are estimated by the N3 analysis using James–Stein shrinkage
(§5.2). No hardcoded priors are used — all values are derived from the data.
The shrinkage target is the observation-weighted mean of the fitted LoB betas.

For LoB with fewer than `min_obs_per_lob` observations, the standard error is
artificially inflated, ensuring strong shrinkage toward the overall mean.
...
Suppose the N3 analysis estimated (via RE-GLS + James–Stein shrinkage):

- $\beta_{\text{Property}} = -0.45$
- $\beta_{\text{Casualty}} = -0.28$
...
$$
\beta_{\text{weighted}} = \frac{0.60 \times (-0.45) + 0.40 \times (-0.28)}{0.60 + 0.40} = \frac{-0.270 + (-0.112)}{1.0} = -0.382
$$

**Step 3: Size adjustment factor**

$$
A = \left(\frac{200}{500}\right)^{-0.382} = 0.4^{-0.382} = e^{-0.382 \ln 0.4} = e^{-0.382 \times (-0.9163)} = e^{0.3500} \approx 1.419
$$

**Step 4: Adjusted severity**

$$
S_{\text{adjusted}} = 0.122 \times 1.419 \approx 0.173 \quad (17.3\%)
$$
...
> **Note:** The $\beta$ values above are illustrative. In practice, all
> coefficients are estimated from the data by the N3 analysis — there are no
> hardcoded defaults.
...
| `winsorisation` | `"none"` | No winsorisation applied to raw severity (preserves tail for VaR 99.5%) |
| `bootstrap_replicates` | 500 | Number of bootstrap resamples for confidence intervals |
| `bootstrap_seed` | 42 | Random seed for reproducibility |
| `leave_out_iterations` | 200 | Number of leave-out resampling iterations (N0 robustness) |
| `leave_out_fraction` | 0.10 | Fraction of syndicates removed per leave-out iteration |

### 8.3  LoB weight floor

All LoB weights are floored at 1% to prevent division by near-zero exposures
when computing LoB-level severity (§2.3):

$$
w_\ell = \max(w_\ell,\; 0.01)
$$
...
The unified analysis table (built within `run_analysis.py`) assembles one row
per syndicate-year observation from the individual extraction files.

### 9.1  Data sources

Data is loaded from individual syndicate-year extraction files:

| Source | File pattern | Contents |
|--------|-------------|----------|
| Syndicate extractions | `pdf_extraction/syndicate_NNNN_YYYY.json` | Complete extraction per syndicate-year: reserve movements, LoB weights (premium mix), severity ratios, directions, narratives, validation results, data quality tags |

Each JSON file is classified during loading into one of five data quality tags:
**RELIABLE**, **INCOMPLETE**, **IN RUNOFF**, **SKIPPED**, or **EXCLUDED**. Only
RELIABLE records are used in the main analysis pipeline.
...
Estimates the size-severity elasticity using the **RE-GLS primary estimator**
(§5.1) plus four comparison model specifications:

| Model | Specification | Purpose |
|-------|---------------|---------|
| **Primary (RE-GLS)** | $s_{it} = \alpha + \beta \ln R_{it} + u_i + \gamma_e + \varepsilon_{it}$ | Syndicate random effects + event FE |
| **M0** | Simple OLS: $s_i = \alpha + \beta \ln R_i$ | Baseline (no controls) |
| **M1** | OLS + event FE | Controls for common events |
| **M2** | Event FE on $\log|S|$ | Log-scale (absolute severity) |
| **M3** | Event FE on $|S|$ | Variance-scale (absolute severity) |
| **M1\_balanced** | M1 on BALANCED\_K8 subset | Balanced panel robustness |
...
VaR 99%, VaR 99.5%, TVaR 99%, and TVaR 99.5% are computed for each
distribution.

**Shapley attribution:**

The mix and size effects are decomposed using Shapley values to account for
interaction effects:

$$
\text{Mix effect} = \tfrac{1}{2}\bigl[(V_{\text{mix}} - V_{\text{naive}}) + (V_{\text{full}} - V_{\text{size}})\bigr]
$$

$$
\text{Size effect} = \tfrac{1}{2}\bigl[(V_{\text{size}} - V_{\text{naive}}) + (V_{\text{full}} - V_{\text{mix}})\bigr]
$$

where $V_x$ is the capital metric (VaR or TVaR) for distribution $x$. This
ensures the attribution is symmetric and the two effects sum exactly to the
...
- **HHI direction analysis**: tests whether concentrated portfolios exhibit
  higher severity variance
- **Power-law dispersion models**: fits $\text{Var}(s^2) = A + B \cdot
  \text{HHI}^C$ and joint models incorporating both HHI and size
- **LoB dispersion parameters**: estimates per-LoB variance contributions via
  NNLS (non-negative least squares), winsorised at p95 on squared severity

#### N6 — Joint Composition (Combined Model)

Fits a sequential size→HHI adjustment pipeline:

1. Fit power-law dispersion on size: $V_{\text{size}}(R) = A_1 + B_1 \cdot R^{C_1}$
2. Compute size-adjusted residuals
3. Fit HHI dispersion on residuals: $V_{\text{hhi}}(\text{HHI}) = A_2 + B_2 \cdot \text{HHI}^{C_2}$
4. Combined adjustment: $V(s^2) = V_{\text{size}}(R) \times V_{\text{hhi}}(\text{HHI}) / V_{\text{hhi}}(\text{HHI}_{\text{ref}})$

The resulting `COMBINED_MODEL` is used by the `size_factor()` function when
available, replacing the simple power-law with a 2D adjustment that accounts
for both portfolio size and LoB concentration simultaneously.

#### Reference mean test

Before computing capital metrics, a **reference mean test** determines whether
the mix-standardised distribution should be re-centred:

$$
\mu_{\text{ref}},\; \text{is\_significant},\; t,\; p = \text{t-test on standardised values}
$$

If the mean is significantly different from zero, the standardised distribution
is re-centred to remove spurious mean shift introduced by the nonlinear
projection:

$$
S_{\text{recentred},i} = S_{\text{std},i} - \overline{S}_{\text{std}} + \mu_{\text{ref}}
$$
...
See §3.4 for the Hellinger distance methodology. At each threshold
$H_{\max} \in \{0.30, 0.35, \ldots, 1.0\}$, VaR 99.5% is computed for both
raw and adjusted severity distributions, along with donor counts. This
analysis validates that the LoB-mix adjustment correctly removes composition
distortion.
...
In addition to the grid portfolios, five **persona portfolios** are analysed
to represent archetypal market participants:

| Persona | Description |
|---------|-------------|
| **Typical** | £500m reserves, equal-weighted LoB mix |
| **Small** | £200m reserves |
| **Large** | £2bn reserves |
| **Diversified** | Low HHI (many LoB with significant weight) |
| **Undiversified** | High HHI (concentrated in few LoB) |

Each persona receives the full four-distribution analysis (naive, mix-only,
size-only, full adjustment) with VaR, TVaR, and Shapley decomposition.

---

## 12  Outputs

### 12.0  Interactive dashboard

The file `pdf_extraction/exposure_analysis.html` is an interactive browser-based
dashboard for reviewing exposure adjustment results. It loads
`exposure_results.json` (produced by `run_analysis.py`) and renders 16 tabs:
Data Summary, Distribution, Tail Trends, Tail Diagnostics, Size Scaling,
Exposure Composition, Joint Composition, Capital Impact, Robustness, 5 Personas,
Worked Example, Data Quality. Features include colour-blind mode toggle,
...
```
                     SYNDICATE EXTRACTION FILES
                     ──────────────────────────
                     pdf_extraction/syndicate_NNNN_YYYY.json
                                   │
                          ┌────────┼────────┐
                          ▼        ▼        ▼
                    Data Quality  LoB     Reserve
                    Classification Weights  Base &
                    (RELIABLE/    (premium  Movements
                     INCOMPLETE/   mix)
                     RUNOFF/...)
                          │        │        │
                          ▼        └───┬────┘
                    ...
                    only               │
                          ...
                          ┌─────────────────────┼─────────────┘
                          ▼                     ▼
                   DIRECT PROJECTION      SIZE COEFFICIENT
                                          ESTIMATION
                          │               (RE-GLS + shrinkage)
                          ▼                     │
              S_std = Σ w_ℓ^(q) · s_ℓ          ▼
                          │            β_weighted = Σ w_ℓ^(q) · β_ℓ
                          │                     │
                          └──────────┬──────────┘
                                     ▼
                      S_adj = S_std · (R^(q) / R_ref)^β_weighted
                                     │
                          ┌──────────┼──────────┐
                          ▼          ▼          ▼
                    COMBINED    REFERENCE   CAPITAL
                    MODEL       MEAN TEST   METRICS
                    (N6: size   (re-centre  (VaR, TVaR,
                     × HHI)     if sig.)    Shapley)
                          │          │          │
                          └──────────┼──────────┘
                                     ▼
                          exposure_results.json
                     (consumed by exposure_analysis.html)
```
...
All analysis code has been consolidated into a single script. The previous
multi-module structure (`scripts/stress_test/...`) no longer exists.
...
| `exposure_results.json` | Complete analysis output bundle (consumed by exposure_analysis.html) |
