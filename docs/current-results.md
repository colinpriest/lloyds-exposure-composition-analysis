...
| concentration exponent $\gamma$ | 0.192 |
| undiversifiable floor $\sigma_{\text{undiv}}$ | 0.0212 |
| diversifiable scale $\sigma_{\text{div}}$ | 0.0606 |
| clean-regime tail $\nu_{\text{clean}}$ | 3.10 |
| RITC-regime tail $\nu_{\text{RITC}}$ | 2.42 |
| RITC tail shift $\lambda_{\text{RITC}}$ | 0.268 |
| RITC scale term $\beta_{\text{RITC}}$ | -0.126 |

Fitted on n = 752 syndicate-years (129 RITC) across 11 reporting years, seed 42. Diagnostics: 0 divergences, max $\hat R$ = 1.00, min bulk ESS = 2417.0.

## What the posterior does and does not settle

| Statement | Value | Status |
|---|---:|---|
| $P(\nu_{\text{RITC}} < \nu_{\text{clean}})$ | 0.848 | RITC tails are heavier |
| $P(\nu_{\text{RITC}} < 2)$ | 0.251 | posterior probability that the RITC regime lacks a finite variance |
| $P(k < 1)$ | $1$ by construction | **tautological** on the bracketed support $[\tfrac12,1]$; stated structurally, not computed from draws |
| $P(k > \tfrac12)$, unconstrained refit | 0.983 | against a prior of 0.50 |
...
| $P(|\beta_{\text{RITC}}| > 0.1)$ | 0.615 | fitted in the likelihood; the transfer operator omits it, not shown to be zero |
...
Loading $\psi_s$ = -0.016, $P(\psi_s > 0)$ = 0.479. This is a **linear loading coefficient on centred log effective size**, not a power elasticity.
...
Source: `results/check_syndicate_random_effect_results.json`. $\tau_\alpha$ = 0.052 against $\sigma_{\text{div}}$ = 0.061 at the reference size (ratio 0.85): persistent between-syndicate level differences are real and material.

## Missingness

Source: `results/missingness_check_results.json`. These figures are read from that file; prose copies of them drift and have.

- 1065 filings, 924 extracted successfully, **141 without the reserves field the diagnostic needs**. That is not the same count as the wholly empty extractions reported in the collection flow, and the two have been conflated before.
- Syndicates with at least one failed year: median size \pounds121.8m against \pounds365.4m for never-fail syndicates ($p = 0.0007$).
- Failed filings' syndicates are smaller than successful ones: \pounds111.0m against \pounds328.7m. **37 orphan filings** come from syndicates never observed at all, so no outcome exists for them by construction.
- Dispersion given size, failure-prone indicator: coefficient -0.0136, $p = 0.141$. **No association was detected among syndicates observed at least once.** That is the whole of what this diagnostic supports: a failure to reject is not a demonstration, and it is silent about the orphans, so **missing-at-random cannot be established**.

Two sensitivities are reported instead of resting on it. Inverse-probability weighting leaves the fit essentially unchanged. The high-volatility orphan stress moves the conditional bracketed estimate from $k = 0.589$ at $c=1$ to $0.562$ at $c=5$ --- a construction that makes the predominantly small missing books more volatile, so it cannot test the adverse-to-sub-linearity direction --- and moves the concentration exponent and the clean-regime tail materially, so the tail is **not** unaffected. See the manuscript for both.
