...
- whether the long-tail share matters for transfer: its slope is resolved positive ($\beta_{\text{LT}} = +0.36$ $[+0.08, +0.64]$) but it does not improve prediction of unseen syndicates (the composition model without the share scores $+1.1$ higher by-syndicate held-out ELPD than with it, Bayesian-bootstrap 95% credible interval $[-7.2, +10.5]$, $P = 0.58$ that the model without it predicts better), and the operator does not carry it;
