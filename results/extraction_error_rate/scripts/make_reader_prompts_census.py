r"""First readers' prompts for the census of the two error mechanisms (protocol, fifth amendment, point 1).

The same instruction the second sample's and the tail's first readers have (reader-prompt-r209.txt), with the batch
named as the census's and the output path error-rate-verdicts-census-batch-N.json. Each substitution must match
exactly once in the template.

    python make_reader_prompts_census.py
"""
import glob
import io
import json
...
batches = sorted(glob.glob(str(SCR / "error-rate-briefs-census-batch-*.json")),
                 key=lambda p: int(re.search(r"batch-(\d+)\.json$", p).group(1)))
if not batches:
    raise SystemExit("no error-rate-briefs-census-batch-*.json: run make_adjudication_briefs_census.py first")
for path in batches:
    ...
    t = once(t, "for a re-reading batch of an extraction error-rate study",
             "for batch %d of the census of two error mechanisms in an extraction error-rate study" % n)
    ...
    t = once(t, "error-rate-verdicts-r209.json", "error-rate-verdicts-census-batch-%d.json" % n)
    out = SCR / ("reader-prompt-census-batch-%d.txt" % n)
    io.open(str(out), "w", encoding="utf-8").write(t)
    print("%s: %d record(s) -> error-rate-verdicts-census-batch-%d.json" % (out.name, k, n))
